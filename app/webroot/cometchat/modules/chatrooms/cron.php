...
if ($_REQUEST['auth'] == $auth) {
    ...
    if (!empty($_REQUEST['cron']) && in_array($_REQUEST['cron'], array('all', 'modules'))) {
        ...
        if (!empty($_REQUEST['cron']) && $_REQUEST['cron'] == "inactiverooms") {
            chatrooms();
        }
        if (!empty($_REQUEST['cron']) && $_REQUEST['cron'] == "chatroommessages") {
            chatroommessages();
        }
        if (!empty($_REQUEST['cron']) && $_REQUEST['cron'] == "inactiveusers") {
            chatroomsusers();
        }
    }
