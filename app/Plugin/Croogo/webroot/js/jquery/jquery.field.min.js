 ...
 */
(function ($) {
    var defaults = {delimiter: ",", checkboxRangeKeyBinding: "shiftKey", useArray: false};
    $.Field = {version: "0.9.2", setDefaults: function (options) {
    ...
    var setValue = function (jq, v) {
        jq.each(function (lc) {
            ...
            switch (t) {
                case"checkbox":
                case"radio":
                    if (valueExists(v, this.value))this.checked = true; else this.checked = false;
                    break;
                case"select":
    ...
    $.fn.autoAdvance = function (callback) {
        return this.find(":text,:password,textarea").bind("keyup.autoAdvance", function (e) {
            var $field = $(this), iMaxLength = parseInt($field.attr("maxlength"), 10);
    ...
    $.fn.moveIndex = function (i) {
        ...
        if (i == "next")i = aPos[0] + 1; else if (i == "prev")i = aPos[0] - 1;
        if (i < 0)i = aPos[1].length - 1; else if (i >= aPos[1].length)i = 0;
    ...
    var getFieldPosition = function (jq) {
        var $field = jq.filter("input, select, textarea").get(0), aTabIndex = [], aPosIndex = [];
        if (!$field)return[-1, []];
    ...
    $.fn.limitSelection = function (limit, options) {
        var opt = jQuery.extend((limit && limit.constructor == Object ? limit : {limit: limit, onsuccess: function (limit) {
            return true
        ...
        var getCount = function (el) {
            if (el.type == "select-multiple")return $("option:selected", self).length; else if (el.type == "checkbox")return self.filter(":checked").length;
            return 0
        };
        ...
        return this.bind((!!self[0] && self[0].type == "select-multiple") ? "change.limitSelection" : "click.limitSelection", function () {
    ...
    $.fn.createCheckboxRange = function (callback) {
        var opt = jQuery.extend((callback && callback.constructor == Object ? callback : {bind: defaults.checkboxRangeKeyBinding, onclick: callback}), callback);
        var iLastSelection = 0, self = this, bCallback = $.isFunction(opt.onclick);
        if (bCallback)this.each(function () {
            opt.onclick.apply(this, [$(this).is(":checked")])
        });
        return this.each(function () {
            ...
            var checkboxClicked = function (e) {
                ...
                if (bCallback)$(this).each(function () {
                    opt.onclick.apply(this, [bSetChecked])
                });
            ...
            $(this).unbind("click.createCheckboxRange").bind("click.createCheckboxRange", checkboxClicked).bind("click.createCheckboxRange", updateLastCheckbox);
            return true
        })
    };
    ...
    var $type = function (o) {
        ...
        if (t == "object") {
            if (o instanceof Array)t = "array"; else if (o instanceof Date)t = "date"
        }
        return t
    };
