...
debug($log); die;*/
    ...
    public function lessons()
    {
        ...
        GROUP BY parent_id) as newest ON Lesson.id = newest.ids WHERE  `Lesson`.`$userLessonConditionsField` = '" . $this->Session->read('Auth.User.id') . "'  AND `Lesson`.`$readConditions` = 0  AND Lesson.lesson_date >= '" . date('Y-m-d') . "'
		");
        ...
        GROUP BY parent_id) as newest ON Lesson.id = newest.ids WHERE  `Lesson`.`$userLessonConditionsField` = '".$this->Session->read('Auth.User.id')."'  AND `Lesson`.`$readConditions` = 0  AND Lesson.lesson_date >= '".date('Y-m-d')."' ";  die;*/
        ...
        GROUP BY parent_id) as newest ON Lesson.id = newest.ids WHERE  `Lesson`.`$userLessonConditionsField` = '" . $this->Session->read('Auth.User.id') . "'  AND `Lesson`.`$readConditions` = 1 AND Lesson.lesson_date >= '" . date('Y-m-d') . "'
		");
...
debug($log); */
    ...
    public function paymentnotmade()
    {
        $redirect = $this->LessonPayment->checkduepayment($this->Session->read('Auth.User.id'));
        $this->Session->write("paymentamount", round($redirect['LessonPayment']['payment_amount']));
        $this->Session->write("paymentstudent", $this->Session->read('Auth.User.id'));
        $this->Session->write("paymenttutor", $redirect['LessonPayment']['tutor_id']);
        $this->set('paymentinfo', $redirect);
    }
