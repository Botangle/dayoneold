...
if (!empty($this->request->data['Usermessage']['attached_files']['tmp_name']) && is_uploaded_file($this->request->data['Usermessage']['attached_files']['tmp_name'])) {
	...
	$profiledir=  	WWW_ROOT .'uploads' . DS . $this->request->data['Usermessage']['send_to']. DS . "message" ;
			...
			if ($this->Usermessage->save($this->request->data)) {
				...
				$this->Session->setFlash(__d('croogo', 'Your Message has been sent successfully.'), 'default', array('class' => 'success'));
				...
				$this->Session->setFlash(__d('croogo', 'This Message cannot be sent. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
		else{
			Croogo::dispatchEvent('Controller.Usersmessage.mesagefailer', $this);
				$this->Session->setFlash(__d('croogo', 'This Message cannot be sent to self. Please, try again.'), 'default', array('class' => 'error'));
		}
	}	 
		if ($username == null) {
		...
		$sent_from = $this->Usermessage->find('all',array('fields'=>'`Usermessage`.`id` as ids,date as date,send_to,sent_from,body,readmessage,attached_files,parent_id,User.username,`User`.`profilepic`,`User`.`id`',
		'joins'=>array(array(
            'table' => 'users',
            'alias' => 'User',
            'type' => 'INNER',
		...
		'conditions'=>array('OR'=>array('AND'=>array('sent_from'=>$this->Auth->user('id'),'send_to'=>$user['User']['id']),array('AND'=>array('send_to'=>$this->Auth->user('id')),'sent_from'=>$user['User']['id']))),'order'=>'ids asc'));
		 
		/* UPDATE MESSAGE AS READ */
		$this->Usermessage->query(" UPDATE `usermessages` SET readmessage = '1' WHERE ((`send_to`= '".$user['User']['id']."' && `sent_from`='".$this->Auth->user('id')."') OR (`sent_from`= '".$user['User']['id']."' && `send_to`='".$this->Auth->user('id')."'))")  ; 
		/*  MESSAGE QUERY */
		...
		$userData  = $this->Usermessage->find('all',array(
		...
		'conditions'=>array('OR'=>array('sent_from'=>$loginUser,'send_to'=>$loginUser)),'group'=>'username','order'=>'id desc'));  
