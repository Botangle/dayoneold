 ...
 */
	...
	function checkpayment(){  
		...
		if($this->Session->check('Auth.User')&& $this->request->params['action']!='paymentnotmade' && $this->request->params['action']!='billing' && $this->request->params['action']!='logout'  && $this->request->params['action']!='updateremaining'){  
...
debug($log); */
	...
	public function updateremaining(){
		...
		if($roletype == 2){
			...
			$lessonPayment = $this->LessonPayment->find('first',array('conditions'=>array('student_id'=>$checktwiddlaid['Lesson']['tutor'],'tutor_id'=>$checktwiddlaid['Lesson']['created']))); 
			if(isset($this->params->query['completelesson']) && ($this->params->query['completelesson']==1)){
